...
class ToggleAssignToTaskTest(TaskTestCase):
    def test_toggle_assign_to_task(self):
        request = self.factory.get(
            reverse("service:toggle-task-assign", args=[self.task.pk])
        )
...
class UpdateTaskProgressTest(TaskTestCase):
    def test_update_task_progress(self):
        ...
        response = self.client.get(
            reverse("service:update-task-progress", args=[self.task.pk])
        )

        self.task.refresh_from_db()
        self.assertTrue(self.task.is_completed)
        self.assertRedirects(
            response, reverse("service:task-detail", args=[self.task.pk])
        )
